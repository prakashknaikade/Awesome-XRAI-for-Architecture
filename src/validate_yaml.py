...
allowed_tags = [
    "2DGS", "3DGS", "360 degree", "Classic Work", "Code", "Dataset", "Diffusion", "Dynamic", "Editing", "Event Camera", "Feed-Forward", 
    "GAN", "Gaussian Video", "Inpainting", "In the Wild", "Language Embedding", "Large-Scale", "Lidar", "LoD", 
    "Meshing", "Misc", "Monocular", "Perspective-correct", "Object Detection", 
    "Optimization", "Physics", "Point Cloud", "Poses", "Project", "Ray Tracing", 
    "Rendering", "Relight", "Review", "Segmentation", "SLAM", "Sparse", "Super Resolution",
    "Stereo", "Style Transfer", "Texturing", "Transformer", "Video", 
...
def get_changed_entries():
    ...
    with open("awesome_xrai_architecture_papers.yaml", 'r') as file:
        new_yaml = yaml.safe_load(file)
    
    try:
        # Get base content
        base_content = repo.get_contents("awesome_xrai_architecture_papers.yaml", ref=pr.base.sha).decoded_content.decode()
